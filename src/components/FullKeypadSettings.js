...
const FullKeypad = ({ navigation, code, cursor, setCode, setCursor, itemsRef }) => {
    ...
    const data = capsLock ? rows : rowsSmall
        ...
        <View style={styles.keypadWrap}>
                ...
                <View key={rowIdx} style={[styles.rowCenter]}>
                    ...
                    {row.map((key, keyIdx) => (
                        <TouchableOpacity
                            key={keyIdx}
                            onPress={() => {
                                keyIn(capsLock ? key : key.toLocaleLowerCase());
                            }}
            ...
            <View style={[styles.rowCenter]}>
                ...
                >
                    <LinearGradient style={styles.gradKey} colors={[COLORS.key1, COLORS.key2]}>
                        {
                            capsLock ? <Text style={[styles.textCenter, styles.body2, { color: "green" }]}>Caps lock</Text> :
                                <Text style={[styles.textCenter, styles.body2]}>Caps lock</Text>
                        }
                    </LinearGradient>
                </TouchableOpacity>
                <TouchableOpacity
